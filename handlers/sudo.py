	...
	if text and (type is "private" or (type is "supergroup" or type is "group")) :
		if rank == "sudo":
			...
			if re.search("^رفع نسخه احتياطيه$|^رفع نسخة احتياطية$", text):
				msgID = Bot("sendMessage",{"chat_id":chatID,"text":"انتظر قليلاً يتم تحميل الملف 🖤","reply_to_message_id":message.message_id,"parse_mode":"html","disable_web_page_preview":True})["result"]["message_id"]
		...
		if rank is "sudo" or rank is "asudo":
			...
			if text == c.ADDfiles:
				url = "https://raw.githubusercontent.com/drtpp/ArMoFF-files/master/files"
			...
			if text == c.Ubot:
				...
				for fnh in Files_H:
					url = "https://raw.githubusercontent.com/drtpp/ArMoFF/master/handlers/"+fnh
					out = requests.get(url).text
					f = open("./handlers/"+fnh,"w+")
					f.write(out)
					f.close()
				for fnu in Files_U:
					url = "https://raw.githubusercontent.com/drtpp/ArMoFF/master/utlis/"+fnu
					out = requests.get(url).text
					f = open("./utlis/"+fnu,"w+")
					f.write(out)
					f.close()
				for fnb in Files_B:
					url = "https://raw.githubusercontent.com/drtpp/ArMoFF/master/"+fnb
					out = requests.get(url).text
					f = open("./"+fnb,"w+")
					f.write(out)
					f.close()
				for fnu in Files_L:
					url = "https://raw.githubusercontent.com/drtpp/ArMoFF/master/lang/"+fnu
					out = requests.get(url).text
					f = open("./lang/"+fnu,"w+")
					f.write(out)
					f.close()
