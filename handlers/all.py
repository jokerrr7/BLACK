...
def allGP(client, message,redis):
  ...
  if text :
    ...
    if re.search(c.sors,text):
      kb = InlineKeyboardMarkup([[InlineKeyboardButton("قناه السورس 🖤", url="t.me/BLACK_TEAM_4")],[InlineKeyboardButton("تواصل السورس 🖤", url="t.me/JOK_ER_7bot")],[InlineKeyboardButton("شروحات السورس 🖤", url="t.me/J_F_A_I")]])
      Botuser = client.get_me().username
      Bot("sendMessage",{"chat_id":chatID,"text":r.sors.format("@"+Botuser),"disable_web_page_preview":True,"reply_to_message_id":message.message_id,"parse_mode":"markdown"})
    ...
    if text == "عدد المجموعه" and (rank is not False or rank is not  0 ):
    ...
    if text == "رتبتي":
      t = IDrank(redis,userID,chatID,r)
      Bot("sendMessage",{"chat_id":chatID,"text":f"🖤꒐ رتبتك : {t}","reply_to_message_id":message.message_id,"parse_mode":"html"})
