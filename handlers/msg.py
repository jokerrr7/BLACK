...
def updateHandlers(client, message,redis):
		...
		if text and group is False:
			if (rank is "sudo" or rank is "sudos" or rank is "asudo") or (redis.get("{}Nbot:autoaddbot".format(BOT_ID)) and GPranks(userID,chatID) == "creator"):
				...
				if text == c.add and not redis.sismember("{}Nbot:disabledgroups".format(BOT_ID),chatID) and Ckuser(message):
					...
					kb = InlineKeyboardMarkup([[InlineKeyboardButton(r.MoreInfo, url="t.me/BLACK_TEAM_4")]])
					...
					get = (redis.hget("{}Nbot:links".format(BOT_ID),chatID) or GetLink(chatID) or "https://t.me/BLACK_TEAM_4")
		...
		if text and group is True:
			if (rank is "sudo" or rank is "sudos" or rank is "asudo") or (redis.get("{}Nbot:autoaddbot".format(BOT_ID)) and GPranks(userID,chatID) == "creator"):
				...
				if text == c.disabl  and Ckuser(message):
					...
					kb = InlineKeyboardMarkup([[InlineKeyboardButton(r.MoreInfo, url="t.me/BLACK_TEAM_4")]])
					Bot("sendMessage",{"chat_id":chatID,"text":r.disabl.format(title),"reply_to_message_id":message.message_id,"parse_mode":"markdown"})
		if  group is True:
	...
	if type is "private" and message.outgoing != True:
		...
		if text and re.search("^/start$",text):
			...
			kb = InlineKeyboardMarkup([[InlineKeyboardButton("° sᴏᴜʀᴄᴇ ʙʟᴀᴄᴋ °", url="t.me/BLACK_TEAM_4")]])
			Bot("sendMessage",{"chat_id":chatID,"text":r.botstart.format(getbot.first_name,getbot.username),"reply_to_message_id":message.message_id,"parse_mode":"html"})
